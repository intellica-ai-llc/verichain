...
C4Context
    title System Context for VeriChain (DPD)
    Person(agent_dev, "Agent Developer", "Writes ASL agents, compiles to seedvm bytecode")
    Person(principal, "Human Principal", "Oversees agents, votes, earns premiums")
    Person(capital, "Capital Provider", "Stakes capital on agents")
    Person(regulator, "Regulator", "Verifies compliance proofs")
    System(verichain, "VeriChain (DPD)", "Consensus, settlement, storage, governance, identity")
    System_Ext(polymarket, "Polymarket CLOB V2", "Prediction market exchange")
    System_Ext(kalshi, "Kalshi", "Prediction market exchange")
    System_Ext(hyperliquid, "Hyperliquid HIP‑4", "Binary options on‑chain")
    System_Ext(kucoin, "KuCoin Futures", "Funding rate data")
    System_Ext(lightning, "Bitcoin Lightning Network", "L402 micropayments")
    System_Ext(worldid, "World ID", "Proof‑of‑personhood for UBE genesis")
    System_Ext(supabase, "Supabase", "PostgreSQL database")
    System_Ext(cloudflare, "Cloudflare", "DNS, CDN, Pages hosting for dashboard")
    System_Ext(oracle_cloud, "Oracle Cloud", "Free‑tier VMs")
    System_Ext(phala, "Phala Cloud", "TEE‑secured VMs")
    System_Ext(cudos, "CUDOS Intercloud", "Cheap VMs")
    System_Ext(lqwd, "LQWD AI Launchpad", "Lightning onboarding")
    Rel(agent_dev, verichain, "Compiles agents, deploys seedvm bytecode")
    Rel(principal, verichain, "Uses Oversight Dashboard, votes")
    Rel(capital, verichain, "Stakes via Lightning")
    Rel(regulator, verichain, "Verifies ZK compliance proofs")
    Rel(verichain, polymarket, "WebSocket for order books, CLOB V2 orders")
    Rel(verichain, kalshi, "REST for market data")
    Rel(verichain, hyperliquid, "WebSocket for HIP‑4 markets")
    Rel(verichain, kucoin, "REST/WebSocket for funding rates")
    Rel(verichain, lightning, "L402 payments via LQWD SaturnZap")
    Rel(verichain, worldid, "Verify human uniqueness")
    Rel(verichain, supabase, "Database queries")
    Rel(verichain, cloudflare, "Dashboard hosting, DNS")
1.4 Constraints
...
sequenceDiagram
    participant S2 as S2 Agent (Legislative)
    participant Gov as governance
    participant FGE as fge (EnvelopeComputer)
    participant MACI as MACICoordinator
    participant DKG as dkg-service
    participant DP as dp-store
    
    S2->>Gov: propose_amendment(charter_id, field, new_value)
    Gov->>Gov: Run adversarial simulation (10,000 Monte Carlo trials)
    Note over Gov: Simulation checks safety violations, fiscal breaches
    alt Simulation fails
        Gov-->>S2: Rejected: SimulationFailed
    else Simulation passes
        Gov->>FGE: compute_envelope(proposed_amendment)
        FGE-->>Gov: Envelope (closest safe amendment)
        alt Amendment differs from envelope
            Gov-->>S2: Rejected: AutonomyDecrease
        else Amendment equals envelope
            Gov->>S2: Legislative vote (2/3 supermajority)
            Gov->>MACI: Human ratification vote
            MACI->>DKG: Request threshold decryption (5-of-9)
            DKG-->>MACI: Decrypted tally
            alt 2/3 supermajority achieved
                Gov->>DP: Record amendment, start 14-day time-lock
                Note over Gov: Veto window active
                alt No veto after 14 days
                    Gov->>DP: Enact amendment
                end
            else Supermajority failed
                Gov-->>S2: Rejected
            end
        end
    end
...
SLSA L3 requires verifiable provenance — build platform must be isolated and provenance unforgeable	DIRECT_QUOTE	SLSA v1.0 spec; Broadcom SLSA L3 documentation (Apr 2026)	VERIFIED